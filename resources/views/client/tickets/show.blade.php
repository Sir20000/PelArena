<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-2xl bg-gradient-to-r from-blue-600 via-green-400 to-purple-500 bg-clip-text text-transparent  leading-tight">
                {{ __('Ticket') }} #{{ $ticket->id }}: {{ $ticket->title }}
            </h2>
            <span class="px-4 py-1 text-sm rounded-xl 
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class=" dark:bg-gray-800 shadow-md rounded-xl overflow-hidden ">
                    ...
                    @if ($ticket->status == 'open')
                        <form method="POST" action="{{ route('tickets.message', $ticket->id) }}" class="mt-6">
                            ...
                            <div class="flex items-start gap-4">
                                <textarea name="message" class="flex-grow p-3 border  rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-800" rows="3" placeholder="Votre message..."></textarea>
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
