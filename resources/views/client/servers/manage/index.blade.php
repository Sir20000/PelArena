            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4 flex  items-center">
                                <img src="{{ $categorie->image }}" alt="Catégorie" class="h-10 w-10 rounded-lg mr-4">
                                {{ $categorie->name }}
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                        <ul class=" text-gray-600 dark:text-gray-400 ">
<br>                        <li  class="mt-4">Allocations : {{ $server->allocations }}</li>
                                <li>Backups : {{ $server->backups }}</li>
                                <li>Base de données : {{ $server->db }}</li>
                        </div>
                        </ul>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <h3 class="text-gray-800 mt-1.5 dark:text-gray-200 text-lg">List of Backups {{$nbr}} / {{$server->backups}} </h3>
                        ...
                        <div>
                    <button id="createBackup" class="bg-gray-400 text-white rounded-lg p-2">Create a Backup</button>
                    </div>    
                </div>    
                @if ($backups)
                    @foreach($backups as $backup)
                    <div class="grid grid-cols-12 gap-4 rounded-lg bg-white dark:bg-gray-700 overflow-hidden shadow-sm sm:rounded-lg p-1" id='RANDOMIDLOL' style='display: flex;justify-content: space-between;'>
                        ...
                        <div class="item" style="margin:5px;border-radius:5px;">
                            <button onclick="downloadBackup('{{ $backup['attributes']['uuid'] }}')" class="bg-gray-400 text-white rounded-lg p-2">Télécharger</button>
                            <button onclick="restoreBackup('{{ $backup['attributes']['uuid'] }}')" class="bg-red-400 text-white rounded-lg p-2">Restaurer</button>
                            <button onclick="deleteBackup('{{ $backup['attributes']['uuid'] }}')" class="bg-red-400 text-white rounded-lg p-2">Supprimer</button>
                        </div>
                    </div>
                    @endforeach
                    @else
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        alert(data.message || 'An error occurred while downloading the backup.');
                    }
                })
                .catch(error => {
            ...
            fetch(`{{ route('extensions.pterodactyl.backups.restore', ['product' => $server->id, 'backupId' => '__backupId__']) }}`.replace('__backupId__', backupId), {
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        console.error('Erreur lors de la restauration du backup :', data.message);
                    }
                })
                .catch(error => {
