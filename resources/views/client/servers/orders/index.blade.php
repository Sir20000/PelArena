<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Crée un serveur {{ $categorie->name }}
        </h2>
    </x-slot>

    <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-6">
        @if (session('error'))
        ...
        <form action="{{ route('client.servers.order.create', ['categorie' => $categorie->id]) }}" method="POST" class="">
            ...
            <div class="mb-4  text-center items-center flex flex-col">
                    ...
                    <div class="input-group flex w-96">
                        <input type="number" name="server_name" id="server_name" required  class="form-control rounded-lg mt-1 w-full dark:bg-gray-800 bg-gray-200 text-black dark:text-white border-gray-500 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 max-w-[404px]" />
                    </div>
            ...
            @foreach (['ram' => 'RAM GB', 'cpu' => 'CPU Core', 'disk' => 'Disque GB', 'db' => 'DataBase Mariadb', 'allocations' => 'Allocation', 'backups' => 'Backups'] as $field => $label)
                <div class="mb-4 text-center items-center flex flex-col  w-full">
                    ...
                    <div class="input-group flex  w-96" >
                        <input type="number" name="{{ $field }}" id="{{ $field }}" required min="0" step="0.01" max="{{ $maxValues[$field] }}" class="form-control rounded-lg mt-1 w-full dark:bg-gray-800 bg-gray-200 text-black dark:text-white border-gray-500 focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 " />
    ...
    <script>
        ...
        const prixUnitaire = {
            ...
            disk: {{ $prix->storage }},
            db: {{ $prix->db }},
            allocations: {{ $prix->allocations }},
            backups: {{ $prix->backups }}
        };
