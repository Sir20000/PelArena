...
const prixData = @json($prixData); // Conversion des données PHP en JavaScript

// Extraire les labels (catégories) et les données pour chaque caractéristique
const categories = prixData.map(function(item) {
    return item.categorie.name; // Ou item.categories.name si tu veux les noms
});

const ramData = prixData.map(function(item) {
    return item.ram;
});

const cpuData = prixData.map(function(item) {
    return item.cpu;
});

const storageData = prixData.map(function(item) {
    return item.storage;
});

const dbData = prixData.map(function(item) {
    return item.db;
});

const backupsData = prixData.map(function(item) {
    return item.backups;
});

const allocationsData = prixData.map(function(item) {
    return item.allocations;
});

// Créer le graphique
const ctxa = document.getElementById('prixChart').getContext('2d');
new Chart(ctxa, {
