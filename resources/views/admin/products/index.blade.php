    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl">
                ...
                @if(auth()->user() && auth()->user()->hasAccess('admin.products.create'))
                    ...
                    <button onclick="window.location.href='{{ route('admin.products.create') }}'" class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-800 border border-transparent rounded-md font-semibold text-xs dark:text-white text-gray-800 uppercase tracking-widest dark:hover:bg-gray-700 hover:bg-white dark:focus:bg-gray-700 focus:bg-white dark:active:bg-gray-900 active:bg-gray-300 focus:outline-none focus:ring-2 dark:focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition ease-in-out duration-150" class="btn btn-success custom-success-button">Crée une nouvelle categorie</button>
                    </div>
                    <div class="space-y-4 mt-6 ">
            @foreach($products as $product)
                <div class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-md flex items-center justify-between">
                    <div class="flex items-center">
                                        <img src="{{ $product->image }}" alt="Image de {{ $product->name }}" class="w-16 mr-4 h-16 object-cover">
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-medium text-black dark:text-white">ID : {{ $product->id }}</h3>
                        <p class="text-gray-300 dark:text-gray-600">Nom : {{ $product->name }}</p>
                        <p class="text-gray-300 dark:text-gray-600">Description : {{ $product->description }}</p>
                    </div>
                    <div class="flex items-center space-x-2 ">
                    @if(auth()->user() && auth()->user()->hasAccess('admin.products.edit'))

                        <a href="{{ route('admin.products.edit', $product->id) }}" class="btn btn-warning">Modifier</a>
@endif
@if(auth()->user() && auth()->user()->hasAccess('admin.products.destroy'))

                        <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce coupon ?');">
                            @csrf
                            @method('GET')
